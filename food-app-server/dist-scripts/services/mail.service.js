...
class MailService {
    ...
    async sendVerificationEmail(toEmail, name, token) {
      ...
      `,
        ...
        try {
            ...
            console.log(`[MailService] Preview URL: ${nodemailer_1.default.getTestMessageUrl(info)}`);
        }
        catch (error) {
            console.error(`[MailService] Error sending verification email:`, error);
        }
    }
    async sendComplaintNotification(details) {
        await this.createTestAccount();
